...
var Lightbox = {
  ...
  changeContent: function(html) {
    ...
    // if they added some footer content, copy it to the real footer
    if($('.modal-body .modal-footer').length > 0) {
      $('.modal-content > .modal-footer').html($('.modal-body > .modal-footer').html());
      $('.modal-body > .modal-footer').remove();
      $('.modal-content > .modal-footer').css("display", "block");
    } else {
      $('.modal-content > .modal-footer').css("display", "none");
    }
  ...
  displayError: function(message, type) {
    ...
    // Page without alert
      ...
      $('#modal .modal-body').prepend('<div class="alert alert-'+type+' alert-dismissible" role="alert"><button type="button" class="close" data-dismiss="alert"><i aria-hidden="true" class="fa fa-close"></i><span class="sr-only">Close</span></button><p class="message">'+message+'</p></div>');
    }
    $('.fa-spinner').remove();
